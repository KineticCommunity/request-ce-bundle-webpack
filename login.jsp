...
<c:choose>
    <%-- 
        ...
        If the kapp/space "Login Page" is specified like "login.jsp?fragment=login", then redirect
        to the kapp/space webpack bundle (ie /acme/#/login or /acme/services/#/login).
    ...
    <%-- 
        ...
        If the kapp/space "Login Page" is not specified or is specified like "login.jsp" (implying
        the webpack bundle does not define a login page), then render the default base bundle login
        page.
    --%>
    <c:otherwise>
        <% 
            ...
            request.getRequestDispatcher("/WEB-INF/app/bundle/login.jsp").forward(request, response); 
        %>
    </c:otherwise>
</c:choose>
