...
<c:choose>
    ...
    <%-- 
        ...
        would potentially allow an attacker craft a URL that would load a malicious bundle).
    --%>
    <c:when test="${param.path != null && pageContext.request.queryString.contains(param.path)}">
        <c:set var="staticLocation" value="${bundle.location}/${path}"/>
    </c:when>
</c:choose>
...
<html>
    ...
    <body>
        ...
        <c:choose>
            ...
            <%-- MISSING BUNDLE NAME --%>
            <c:when test="${pageContext.request.getHeader('X-Webpack-Bundle-Name') != null}">
                The display page has not been configured to support development mode, but your 
                request passing an X-Webpack-Bundle-Name header value of 
                "${text.escape(pageContext.request.getHeader('X-Webpack-Bundle-Name'))}".
            </c:when>
            <%-- MISCONFIGURATION --%>
            <c:otherwise>
                ...
                <br/><br/>
                To render this resource in development mode (ie running the webpack bundle from a
                local machine), set the <b>bundleName</b> parameter.
                <br/><br/>
                To render this resource in deployed mode, you must set the <b>location</b> (for
                external asset mode) or the <b>path</b> parameter (for embedded asset mode).
            </c:otherwise>
        </c:choose>
    </body>
</html>
